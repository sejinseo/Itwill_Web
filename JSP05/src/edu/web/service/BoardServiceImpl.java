...
public class BoardServiceImpl implements BoardService {
	...
	public Board select(int bno) {
		System.out.println("boardServiceImpl.select(bno = " + bno + ") 메소드 호출");
		
		Board board = boardDao.read(bno);	//글 번호로 게시글 검색
		if (board != null ) {	//검색 결과가 있을 때
			int result = boardDao.update(bno);	//글 조회수 1씩 증가
		}		
	...
	public int update(Board board) {
		System.out.println("boardServiceImple.update(Board board) 메소드 호출");
	...
	public int delete(int bno) {
		System.out.println("boardServiceImpl.delete(bno = " + bno + ") 메소드 호출");
	...
	public List<Board> select(int type, String keyword) {
		System.out.println("boardServiceImpl.select(type=" + type + ", keyword=" + keyword + ") 메소드 호출");
