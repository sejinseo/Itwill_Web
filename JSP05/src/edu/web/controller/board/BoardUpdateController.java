...
public class BoardUpdateController implements Action {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) {
		System.out.println("boardUpdateController.execute() 메소드 호출");
	...
	private String doPost(HttpServletRequest request, HttpServletResponse response) {
		// 수정된 게시글의 내용을 DB에 update하고 게시판 메인으로 redirect
		System.out.println("boardUpdateController.doPost() 메소드 호출");
		
		int bno = Integer.parseInt(request.getParameter("bno"));
		String title = request.getParameter("title");
		String content = request.getParameter("content");	//param 이름은 jsp파일 내 name과 같다.
