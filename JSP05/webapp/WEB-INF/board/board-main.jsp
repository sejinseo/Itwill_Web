		...
		<div class="container-fluid">
			...
			<div> <!-- 게시판 목록 테이블 -->				
				<table class="table table-hover">
					...
					<tbody>
					<%-- ${boardList}: BoardMainController에서 request 객체에 저장(setAttribute)한 List객체 --%>
						<c:forEach var="b" items="${boardList }">
							<tr>
							<%-- ${b.bno}: Board 객체 b의 getBno() 메소드 호출 결과 --%>
								<td>${b.bno}</td>
								<td>
									<a href="detail?bno=${b.bno}">${b.title}</a> <!-- get방식 : ?n=v, query스트링  -->
								</td>
								<td>${b.userId}</td>
