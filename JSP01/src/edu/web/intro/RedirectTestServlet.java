...
public class RedirectTestServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) 
		...
		System.out.println("redirectTestServlet.doGet() 메소드 호출");
		
		//redirect 방식의 페이지 이동:
		//최초 요청(request)을 받은 페이지가 응답(response)을 처리하지 않음.
		//request 와 response 객체가 유지되지 않음.
		//최초 요청을 받은 페이지에서 새로운 request, response 객체를 생성하고 페이지를 이동.
		//최초 요청 URL이 유지되지 않고 이동되는 페이지주소로 URL 변경
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) 
			throws ServletException, IOException {
		System.out.println("redirectTestServlet.doPost() 메소드 호출");
	}
