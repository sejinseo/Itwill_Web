...
<html>
	...
	<body>		
		...
		<%
		request.setCharacterEncoding("UTF-8");	//POST방식의 데이터 전송에서 한글 처리
		//요청 파라미터(request parameter)의 값을 찾음.
		String param1 = request.getParameter("number1");
		String param2 = request.getParameter("number2");
		
		//요청 파라미터 값들을 int 타입 변환
		...
		EL에서 요청 파라미터 값을 찾는 방법:
			...
			* ${ param['name'] }
		--%>
		${ param.number1 } + ${ param.number2 } = ${ param.number1 + param.number2 } <br>
		${ param.number1 } - ${ param.number2 } = ${ param.number1 - param.number2 } <br>
		${ param.number1 } * ${ param.number2 } = ${ param.number1 * param['number2'] } <br>
		${ param['number1'] } / ${ param.number2 } = ${ param['number1'] / param.number2 } <br>
